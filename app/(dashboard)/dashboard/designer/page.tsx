...
export default async function DesignerDashboard() {
  const profile = await requireRole('designer')
  const supabase = await createClient()

  // Get tasks assigned to this designer with profile info
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-semibold tracking-tight text-foreground">Designer Dashboard</h1>
        <p className="text-muted-foreground mt-2">
          Welcome back, {profile.full_name || 'Designer'}{profile.title ? ` - ${profile.title}` : ''}
        </p>
      </div>
