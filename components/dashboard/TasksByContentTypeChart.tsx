...
interface TasksByContentTypeChartProps {
  data: Array<{ contentType: string; count: number }>
}

export function TasksByContentTypeChart({ data }: TasksByContentTypeChartProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg font-semibold">Tasks by Content Type</CardTitle>
      </CardHeader>
      <CardContent>
        {chartData.length > 0 ? (
